...
{
    $user    = Auth::user();
    $request = RequestModel::with(['requester', 'department', 'assignedExpert', 'files', 'replies.expert'])
        ->findOrFail($id);
...
public function refer(\Illuminate\Http\Request $httpRequest, $id)
{
    ...
    \App\Models\RequestReply::create([
        ...
        'is_referral' => true,
        'referred_to' => $httpRequest->expert_id,
    ]);

    $req->update([
        'status'             => 'referred',
        'assigned_expert_id' => $httpRequest->expert_id,
        'referred_at'        => now(),
    ]);

    $this->notifyExpertByBale($req, $httpRequest->expert_id);
...
public function returnRequest($id)
{
    ...
    $lastReferral = $req->replies()->where('is_referral', true)->latest()->first();
    $returnTo     = $lastReferral?->expert_id;
    ...
    \App\Models\RequestReply::create([
        ...
        'is_referral' => true,
        'referred_to' => $returnTo,
    ]);

    $req->update([
        'status'             => 'referred',
        'assigned_expert_id' => $returnTo,
        'referred_at'        => now(),
    ]);

    $this->notifyExpertByBale($req, $returnTo);
